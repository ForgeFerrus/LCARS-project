...
class LCARSDemo(QMainWindow):
    def __init__(self):
        ...
        self.button_timers = {}
        self.setup_ui()
        # Консоль оновиться після створення UI
    ...
    def setup_ui(self):
        ...
        # Кнопки епох - тільки для Starfleet
        ...
        self.eras_label = QLabel("--- STARFLEET ERAS ---")
    ...
    def update_system_console(self):
        ...
        console_text += f"• MODE - Toggle constructor mode\n"
        console_text += f"• BACK - Return to welcome\n\n"
        
        if self.edit_mode:
            console_text += f"═══════════════════════════════════════\n"
            console_text += f"    CONSTRUCTOR MODE ACTIVATED\n"
            console_text += f"═══════════════════════════════════════\n\n"
            console_text += f"🔧 INTERFACE EDITING TOOLS:\n"
            console_text += f"• Modify widget properties\n"
            console_text += f"• Change layout parameters\n"
            console_text += f"• Adjust styling options\n"
            console_text += f"• Export interface design\n"
            console_text += f"• Save custom themes\n\n"
            console_text += f"CONSTRUCTOR FEATURES:\n"
            console_text += f"• Drag & drop widgets\n"
            console_text += f"• Real-time preview\n"
            console_text += f"• Property inspector\n"
            console_text += f"• Theme editor\n"
            console_text += f"• Layout manager\n\n"
            console_text += f"STATUS: READY FOR EDITING\n"
            console_text += f"═══════════════════════════════════════"
        else:
            console_text += f"═══════════════════════════════════════"
        
        self.palette_display.setPlainText(console_text)
        
    def toggle_mode(self):
        self.edit_mode = not self.edit_mode
        mode_text = "CONSTRUCTOR MODE ENABLED" if self.edit_mode else "DISPLAY MODE"
        ...
        if self.edit_mode:
            ...
            ''')
            self.faction_welcome.setText("CONSTRUCTOR MODE - Interface editing enabled")
        else:
    ...
    def go_back(self):
        ...
        while self.colors_container_layout.count():
            child = self.colors_container_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()
        
        # Додати colors_layout назад в контейнер
        self.colors_container_layout.addLayout(self.colors_layout)
    ...
    def select_faction(self, faction, greeting):
        ...
        # Показати епохи для кожної фракції
        if faction == 'starfleet':
            self.eras_frame.setVisible(True)
            self.eras_label.setText("--- STARFLEET ERAS ---")
            # Оновити кнопки епох для Starfleet
            self.update_era_buttons('starfleet')
        elif faction == 'klingon':
            self.eras_frame.setVisible(True)
            self.eras_label.setText("--- KLINGON ERAS ---")
            # Оновити кнопки епох для Klingon
            self.update_era_buttons('klingon')
        elif faction == 'romulan':
            self.eras_frame.setVisible(True)
            self.eras_label.setText("--- ROMULAN ERAS ---")
            # Оновити кнопки епох для Romulan
            self.update_era_buttons('romulan')
        elif faction == 'cardassian':
            self.eras_frame.setVisible(True)
            self.eras_label.setText("--- CARDASSIAN ERAS ---")
            # Оновити кнопки епох для Cardassian
            self.update_era_buttons('cardassian')
        else:
            self.eras_frame.setVisible(False)
    ...
    def update_era_buttons(self, faction):
        # Очистити існуючі кнопки епох
        for btn in self.era_buttons:
            # Зупинити анімацію перед видаленням
            if id(btn) in self.button_timers:
                self.button_timers[id(btn)].stop()
                del self.button_timers[id(btn)]
            btn.deleteLater()
        self.era_buttons.clear()
        
        # Створити нові кнопки епох для фракції
        if faction == 'starfleet':
            eras = [
                ("22nd", LCARSEra.COMS_22ND),
                ("23rd", LCARSEra.PCARS_23RD),
            ...
            eras = [
                ("22nd", FactionEra.KLINGON_22ND),
                ("23rd", FactionEra.KLINGON_23RD),
                ("24th", FactionEra.KLINGON_24TH),
                ("25th", FactionEra.KLINGON_25TH)
            ]
        elif faction == 'romulan':
            eras = [
                ("22nd", FactionEra.ROMULAN_22ND),
                ("23rd", FactionEra.ROMULAN_23RD),
                ("23st", FactionEra.ROMULAN_23ST),
                ("24th", FactionEra.ROMULAN_24TH),
                ("25th", FactionEra.ROMULAN_25TH)
            ]
        elif faction == 'cardassian':
            eras = [
                ("22nd", FactionEra.CARDASSIAN_22ND),
                ("23rd", FactionEra.CARDASSIAN_23RD),
                ("24th", FactionEra.CARDASSIAN_24TH),
                ("25th", FactionEra.CARDASSIAN_25TH)
            ]
        else:
            return
    ...
    def select_era(self, era):
        # Вибрати еру для будь-якої фракції
        try:
            # Перевірити тип ери (LCARSEra або FactionEra)
            if hasattr(era, 'value'):  # Це LCARSEra
    ...
    def show_palette(self):
        ...
        # Спочатку очистити всі дочірні віджети
        while self.colors_layout.count():
            child = self.colors_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()
        
        # Також очистити основний контейнер
        while self.colors_container_layout.count():
            child = self.colors_container_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()
    ...
    def animate_button(self, button):
        ...
        def change_color():
            try:
                ...
                # Оновити консоль з інформацією про палітру
                if self.current_palette:
                    self.show_palette()
    ...
    # Додаткові функції для системних кнопок
    ...
    def save_current_theme(self):
        print("Saving current theme...")
        
    def reset_interface(self):
        print("Resetting interface...")
    
    def get_default_console_text(self):
        return """═══════════════════════════════════════
        LCARS SYSTEM CONSOLE
═══════════════════════════════════════

SYSTEM STATUS: ONLINE
LOCATION: EARTH SPACE DOCK
USER: LCARS DEMO OPERATOR
SECURITY LEVEL: ALPHA

AVAILABLE FACTIONS:
• STARFLEET - Federation Protocol
• KLINGON - Warrior Honor
• ROMULAN - Shadow Intelligence  
• CARDASSIAN - Military Order
...
def main():
    ...
    print('🖖 Full functionality demo with dynamic interface')
